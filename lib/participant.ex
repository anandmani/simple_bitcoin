...
defmodule Participant do
  ...
  def init(:ok) do
    {:ok,
     %{
       :blockchain => [],  #Stored as a stack. Last block at index 0
       :utxos => [
  ...
  def create_tx_from_inputs_and_outputs(transaction_inputs, transaction_outputs) do
    ...
    IO.puts "Transaction created"; IO.inspect(tx)
    tx
  end
  ...
  def handle_send_satoshi(state, value, public_key_hash) do
    ...
    cond do
      ...
      true ->
        ...
        #Ignoring change creation
        transaction_output = Transaction.generate_a_tx_out(value, public_key_hash)
        tx = create_tx_from_inputs_and_outputs(transaction_inputs, [transaction_output])
        Bitcoind.receive_transaction(:bitcoind, tx)
        unspent_utxos = state[:utxos] |> Enum.take(-1 * (Enum.count(state[:utxos]) -1 - reduced_index))
        IO.puts "Unspent satoshis"; IO.inspect(unspent_utxos)
